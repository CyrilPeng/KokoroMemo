...
- **AIRP 召回解释契约** - 新增 `/admin/airp-recall-explanation`，汇总最近会话 retrieval trace、最终注入记忆、隔离排除原因和潜在串记风险。
- **AIRP 召回解释面板** - 仪表盘新增官方召回解释入口，展示当前角色、本轮选中记忆、角色/会话/记忆库隔离排除和下一步入口。

### 改进

- **AIRP 召回解释门禁** - CI 与 Release 预检新增 `验证 AIRP 召回解释契约` 步骤，覆盖空状态、正常召回解释和跨角色误召回风险。
...
- **首次 AIRP 成功路径** - 新增首次接入、候选记忆、召回解释、状态板和多角色隔离的验收清单。
- **AIRP 主流程导航收纳** - 侧边栏一级入口保留首次成功路径，将长期记忆和会话管理收进辅助管理。
- **AIRP 首次成功验收面板** - 仪表盘新增最小闭环检查，串联模型配置、角色识别、会话接入、候选审核、批准记忆和状态板入口。
