...
会话状态板是 KokoroMemo 的热记忆层，负责维护“当前正在发生什么”。它不替代长期记忆卡片，也不绕过 `memory_inbox` 审核流程；它只作为当前会话的轻量结构化上下文缓存。

核心表位于 `memory.sqlite`：

```text
conversation_state_items    当前活跃 / 已完成 / 已删除的状态项
conversation_state_events   状态项创建、更新、resolve、投影、注入和失败事件
retrieval_decisions         每轮 Retrieval Gate 是否调用长期召回的决策记录
```

状态类别包括：

```text
scene / key_person / main_quest / side_quest / promise / open_loop / relationship
boundary / preference / location / item / world_state / recent_summary / mood
```
...
状态板可以由 Admin API 手动写入，也可以由 State Updater 在每轮完整回复后自动维护。规则版 State Updater 会保守提取地点、场景、当前任务、承诺和边界；LLM 版 State Updater 要求输出严格 JSON，并经过置信度过滤。

---

## 47. Retrieval Gate / 按需召回门控

Retrieval Gate 用于决定当前请求是否需要调用长期记忆召回链路。支持三种模式：

```text
always  每轮都调用长期召回，兼容旧行为
never   不调用长期召回，只使用会话状态板
auto    根据规则自动判断
```
...
```text
新会话首次请求
用户输入包含“记得 / 还记得 / 上次 / 以前 / 之前 / 约定 / 我们说过”等历史触发词
达到周期性召回间隔
状态板平均置信度低于阈值
没有足够状态板可用时回退召回
```

普通短句或当前剧情推进可以跳过 LanceDB，以降低延迟和成本。每次 Gate 决策都会写入 `retrieval_decisions`，GUI 的 Gate 调试页可查看 `request_id`、用户输入、是否召回、原因和状态板置信度。
...
当 `ADMIN_TOKEN` 或 `server.admin_token` 配置后，所有 `/admin/*` 接口都要求：

```http
Authorization: Bearer {ADMIN_TOKEN}
```

GUI 新增“会话状态板”页面，用于按分组查看状态项、手动新增/编辑/resolve/delete 状态项、从 approved boundary/preference 记忆卡片投影长期边界和偏好，并查看 Retrieval Gate 调试记录。
...
7. 必要时执行长期记忆召回、图扩展和可选 Rerank
8. 注入长期记忆候选
9. 转发真实 LLM，保持非流式 / 流式兼容
10. 保存 raw response 和完整 messages
11. 后台执行 State Updater
12. 后台执行 Memory Extractor，长期记忆仍进入 memory_inbox 审核
```
...
桌面端使用 Tauri 2 承载 Vue 管理界面，并负责在应用启动时拉起本地后端服务。发行版的目标是让普通用户双击桌面程序即可使用，不要求用户手动启动 `python -m app.main`。

### 50.1 后端启动策略

```text
源码后端调试：直接运行 python -m app.main
Tauri 开发调试：优先使用 sidecar，缺失时回退到 python -m app.main
Linux / macOS 发行版：随包携带平台后端 sidecar，由 Tauri 启动
Windows 发行版：后端二进制编译进主程序 exe，运行时由主程序释放并启动
```

后端默认监听 `127.0.0.1:14514`，桌面端启动后应保证 OpenAI-compatible API 可被本机客户端访问。发行版启动失败时，应优先检查后端进程是否被拉起、端口是否被占用、运行目录是否可写。

### 50.2 Windows 单 exe 分发原则

Windows 发行版必须遵循单 exe 原则：

```text
Portable.zip 解压后只包含 KokoroMemo.exe
MSI 安装后只安装内嵌后端的 KokoroMemo 主程序
发行包不得额外分离携带 kokoromemo-server.exe
```

实现方式：CI 先构建 Windows 后端 onefile 二进制，再复制到 `gui/src-tauri/embedded-backend/kokoromemo-server.exe`。`build.rs` 检测该文件后启用 `kokoromemo_embedded_backend` 编译条件，Rust 主程序通过 `include_bytes!` 将后端写入主 exe。Windows 专用 Tauri 配置将 `bundle.externalBin` 置空，避免 MSI 或便携版重新打入外部 sidecar。

运行时，主程序会把内嵌后端释放到应用数据目录的 `runtime/kokoromemo-server.exe`，以无控制台窗口方式启动，并在应用退出时结束该子进程。释放出的运行时文件属于内部缓存，不属于发行包内容。
...
关闭主窗口：隐藏窗口，后端继续运行
托盘显示窗口：恢复并聚焦主窗口
托盘退出：设置退出标记，关闭窗口并结束后端进程
设置页开关：允许用户关闭“关闭后最小化到托盘”行为
```

该行为避免用户误关窗口导致本地代理服务中断。真正退出应用时必须同时清理后端子进程，避免 `14514` 端口被僵尸进程占用。

### 50.4 更新检测

设置页提供 GitHub Release 更新检测：
...
对比 tag_name 与当前版本
展示已是最新 / 可更新 / 检查失败状态
提供手动检查和打开 Release 页面入口
```

更新检测只负责提示用户下载新发行版，不在应用内自动覆盖安装。网络请求失败时不得影响本地后端、记忆管理或代理转发功能。

### 50.5 Release 产物命名

GitHub Actions 产物命名遵循：

```text
KokoroMemo-版本号-系统-CPU架构.扩展名
```

当前发行产物约定：

```text
KokoroMemo-0.2.1-Windows-Portable.zip
KokoroMemo-0.2.1-Windows-x64.msi
KokoroMemo-0.2.1-macOS-arm64.dmg
KokoroMemo-0.2.1-Linux-x64.AppImage
```

Windows Portable 使用 `Portable` 表达分发形态，不再额外添加 CPU 架构；MSI、macOS、Linux 产物保留架构字段。后续修改工作流时，不得破坏 Windows 单 exe 内嵌后端约束。
