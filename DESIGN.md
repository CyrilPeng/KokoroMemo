...
| 发布 | Nuitka 编译 + GitHub Actions 三平台 CI + Android 单包压缩包 |
...
Android 单包通过 `KOKOROMEMO_WEB_DIST` 指向包内 `webui/dist`，避免在手机上运行 Node 构建。
...
Android 发布遵循“CI 内部分层构建，Release 外部单包交付”的原则：用户只下载一个与运行环境和 CPU 架构匹配的 `.tar.gz`，包内自带源码、预构建 Web UI、脚本和对应目录结构。

面向用户的 Release 产物：

| 包名 | 用途 |
|---|---|
| `KokoroMemo-vX.Y.Z-Android-Termux-aarch64.tar.gz` | 推荐包，适合大多数现代 Android / Termux 原生环境 |
| `KokoroMemo-vX.Y.Z-Android-ProotUbuntu-aarch64.tar.gz` | 兼容包，适合 Termux 原生依赖安装失败后在 proot Ubuntu 中运行 |

包内结构：

```text
KokoroMemo-X.Y.Z-Android-Termux-aarch64/
├─ install.sh / start.sh / stop.sh / restart.sh / backup.sh / doctor.sh
├─ VERSION
├─ README-Android.md
├─ config.example.yaml
├─ kokoromemo/        # 后端源码与 requirements/android-*.txt
├─ webui/dist/        # CI 预构建前端产物
└─ wheels/<runtime>-aarch64/  # 预留依赖 wheel 目录
```

运行策略：

- Android 端默认使用 Web UI 模式，不构建 Tauri。
- 安装脚本不执行 `npm install` 或 `npm run build`。
- Termux 包优先使用 `pkg install python-numpy` 获取原生 numpy，减少 pip 编译失败。
- proot Ubuntu 包使用 `requirements/android-proot-ubuntu.txt`，可逐步补充 manylinux aarch64 wheel。
- 后端启动脚本设置 `KOKOROMEMO_WEB_DIST=$ROOT/webui/dist` 和 `KOKOROMEMO_CONFIG=$ROOT/config.yaml`。
- `doctor.sh` 用于检查架构、虚拟环境、Web UI、配置和 Python 模块。

GitHub Actions 中 `build-android-packages` job 会在 CI 构建 `gui/dist`，再分别组装 Termux 与 ProotUbuntu 单包并上传为 Release asset。中间产物不单独暴露给普通用户，避免 Release 页面散落 WebUI、wheel、source 等依赖文件。
...
3. Gitee 镜像：通过 `https://gitee.com/api/v5/repos/CyrilPeng/KokoroMemo/releases/latest` 获取最新 tag，再读取该 tag 下的 `latest.json`
