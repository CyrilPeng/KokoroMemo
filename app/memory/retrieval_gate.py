...
def decide_retrieval(gate_input: RetrievalGateInput) -> RetrievalGateDecision:
    ...
    if (
        every_n > 0
        and gate_input.turn_index is not None
        and gate_input.turn_index > 0
        and gate_input.turn_index % every_n == 0
    ):
    ...
    if (
        ...
        and avg_confidence < gate_input.vector_search_when_state_confidence_below
    ):
