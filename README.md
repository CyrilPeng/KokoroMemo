...
GUI 将状态板管理拆成会话选择、表格状态、注入预览和填充调试：

- **会话选择/加载**：输入 `conversation_id` 后加载当前会话状态，避免不同会话数据混淆。
- **表格状态**：按当前场景、角色状态、关系状态、扮演规则、承诺任务、重要事件和重要物品分表管理。
- **行级编辑**：支持新增、编辑、删除状态行，避免把多个信息堆到同一个大文本字段。
- **注入预览**：展示实际注入到模型的状态板文本和字符预算。
- **填充调试**：可手动输入一轮用户消息与助手回复，运行表格化 State Filler 检查更新结果。
...
从 v0.6.0 开始，会话状态板主流程重构为“表格模板 + 行级状态 + 操作式更新”：
