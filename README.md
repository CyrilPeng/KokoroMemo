...
<p align="center">
  <em>面向 AIRP 的本地角色连续性引擎：不忘、不串、不乱记。</em>
</p>
...
**KokoroMemo（心忆）** 是一个面向 **AI 角色扮演（AIRP）**、AI 游戏、AI 桌宠、SillyTavern 类前端与其他 OpenAI-compatible 客户端的 **本地角色连续性引擎**。

它不是又一个通用个人记忆助手，也不是企业知识库 RAG。KokoroMemo 关注的是 AIRP 用户真正容易崩体验的三件事：

- **不忘**：角色能跨会话记住称呼、偏好、边界、关系进展、重要约定和共同经历。
- **不串**：不同角色、不同会话、不同世界观和不同记忆库之间尽量避免互相污染。
- **不乱记**：临时剧情、玩笑、误会和模型单方面编造的内容不会轻易变成长期记忆。

KokoroMemo 作为一个运行在本机的 OpenAI-compatible 代理，位于你的 AI 客户端与真实大模型 API 之间。客户端仍然像往常一样请求聊天模型，KokoroMemo 在中间负责保存对话、维护会话状态、检索长期记忆，并把必要的上下文注入给模型。

它的目标不是把聊天记录简单塞进向量库，而是用可审阅的记忆卡片、表格化会话状态板、按需召回、本地语义索引和检索解释，让 AI 角色在长时间互动中保持人物关系、剧情状态和扮演规则的连续性。
...
- **SillyTavern / Cherry Studio / 其他 OpenAI-compatible AIRP 客户端**：需要让角色长期保持关系、称呼、边界和共同经历。
- **RimTalk / AI 游戏 / 桌宠**：需要维护当前场景、角色状态、任务、物品、资源等热状态，但不希望把临时游戏状态写成长期记忆。
- **跑团 / 长剧情模拟**：需要追踪 NPC、地点、线索、阵营、剧情旗标和承诺任务。
- **本地优先与可审核用户**：希望知道哪些内容被记住、哪些内容被召回，以及为什么没有召回。

KokoroMemo 不优先服务：

- 企业知识库 RAG、文档问答或通用资料库检索。
- 通用个人助理记忆、日程管理或生活日志。
- 云端托管记忆服务、多 Agent 编排平台或商业化记忆 SDK。
- 需要完全无审核、全自动吸收一切聊天记录的场景。
...
- **角色连续性记忆**：对话、记忆卡片、审核记录和状态板默认保存在本机 SQLite 数据库中。
- **会话状态板**：记录“当前正在发生什么”，例如场景、角色状态、关系、规则、任务、事件和物品。
- **可审核长期记忆**：新提炼出的记忆会进入收件箱，可自动通过、人工审核或拒绝，避免误记忆污染角色。
- **隔离与挂载**：按用户、角色、会话和记忆库控制可见范围，降低跨角色、跨世界观和跨库污染。
- **按需召回与解释**：通过 Retrieval Gate 判断是否需要检索长期记忆，并记录召回来源、策略和注入结果。
...
1. 打开 GUI 的“设置 → 接入向导”。
2. 选择最接近的玩法：普通角色扮演、SillyTavern 长会话、RimTalk、跑团剧情或纯代理。
3. 在“模型配置”中填写聊天模型的 Provider、Base URL、API Key 和模型名，并测试连接。
4. 复制“OpenAI Base URL”到你的 AIRP / SillyTavern / OpenAI-compatible 客户端。
5. 先进行一小段普通聊天，再回到 KokoroMemo 查看“总览”。
...
查看角色连续性总览、最近会话、当前角色、状态板行数、长期记忆写入策略、注入/召回策略、待审核数量和服务状态。
...
角色中心用于管理每个角色的档案、默认会话策略、召回策略、记忆库绑定和相关会话。你可以为角色设置显示名称、别名、备注，并为该角色的新会话指定默认方案：
...
状态板用于维护当前会话的短期连续性。v0.6.0 起，默认使用表格化状态板，并在页面顶部展示连续性摘要：
...
你可以按表查看、新增、编辑和删除状态行，也可以查看实际注入到模型的状态板预览。预览侧栏会展示每张状态表是否进入 prompt、纳入了多少行，以及是否因为字符预算被截断。

状态板现在支持“会话方案”“召回策略”和“新会话默认配置”：
...
召回策略用于控制长期记忆检索规模、注入数量、字符预算和检索门控周期，内置“保守召回 / 平衡召回 / 高召回 / 状态板优先 / 记忆优先”。会话状态板右侧的“注入来源”会显示最近聊天的检索 trace，包括本轮使用的召回策略、挂载库、最终注入的记忆卡片、来源库和分数，便于排查漏召回、错召回或跨角色/跨库污染。

如果你准备开始 RimTalk 或跑团，建议先在“状态板”里设置“新会话默认配置”。这样第一次出现的新 `conversation_id` 就会使用正确模板和策略，而不是先落到通用 AIRP 模板。

### 记忆审核

记忆审核页用于查看候选记忆，决定是否批准、拒绝或修改后保存。候选会按风险、类型和审核类别辅助筛选，例如边界规则、关系变化、角色连续性、剧情状态和可能误记。
...
会话管理用于查看客户端连入过的会话，修正角色归属、归档旧会话、预览最近消息，以及导入导出会话配置。
