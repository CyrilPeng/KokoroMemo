...
- “首选监听端口”才是可编辑配置项；修改后需要重启后端才能重新尝试绑定。
...
默认表格模板为 `tpl_rimtalk_roleplay_tables`，面向 RimTalk/连续角色扮演：
...
另有 `tpl_roleplay_light_tables` 作为轻量陪伴/RP 模板。
